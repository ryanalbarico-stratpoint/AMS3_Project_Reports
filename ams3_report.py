#!/usr/bin/env python3
"""
AMS3 Developer Contribution Report generator.
...
Usage:
  python ams3_report.py --init                          # create ams3_report_config.json
  python ams3_report.py --repo OWNER/REPO --discover     # list raw git identities (to fill the config)
  python ams3_report.py --repo OWNER/REPO                # build the contribution report
  python ams3_report.py --init-kpi                       # write the bundled KPI dashboard template

Templates are bundled directly in ams3_template.py and rendered from memory —
...
from ams3_template import CONTRIBUTION_TEMPLATE, KPI_TEMPLATE

CONFIG_FILE = "ams3_report_config.json"
KPI_FILE = "ams3-developer-kpi-latest.html"
MAX_WORKERS = 12
...
DEFAULT_CONFIG = {
    "github_repo": "stratpoint-engineering/mptc-ams3",
    "branch": "develop",
    "start_date": "2026-08-10",
    "tasks_dir": "docs/dev-tasks",
    "output_dir": "reports",
    "modules": {"core": "core", "security": "security", "inventory": "inventory",
                "exempt": "exempt", "audit": "audit"},
    "people": {n: {"emails": [], "git_names": [], "github": []} for n in [
        "Bryan Matias", "Dardee Jay Calura", "Jan Rudolf Basoc", "Jerwin Ordillano",
        "Karl Manuel Barce", "Kianna Alexandra Gragg", "Maureen Reyes", "Pao Rogelio",
        "Ronaldo Dela Cruz", "Steven Palabyab"]},
    "exclude": {
...
def load_template():
    if not DATA_RE.search(CONTRIBUTION_TEMPLATE):
        sys.exit("CONTRIBUTION_TEMPLATE (ams3_template.py) doesn't contain the expected "
...
# ---------------------------------------------------------------- cli
def main():
    ap = argparse.ArgumentParser(description="Build the AMS3 Developer Contribution Report")
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    latest = out_dir / "ams3-dev_contribution_report-latest.html"
    dated = out_dir / f"ams3-dev_contribution_report-{dt.date.today().isoformat()}.html"
